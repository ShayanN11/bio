...
import urllib
import io
#@mr_null_chanel  Youtube Mr Null

auth = input("wqnozomusfspqiunygqbajsmilgtqjdl")
bot = Bot(auth)
target = input("g0Bhu1b08e4c85e20b7ff06b4347f310")

print("Welcome Youtube : Mr Null / Chanel Telegram : @mr_null_chanel")
			...
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						...
						elif msg.get("text").startswith("بای") or msg.get("text").startswith("بایز") or msg.get("text").startswith("بایی") or msg.get("text").startswith("bye"):
							try:
								bot.sendMessage(target,'عه رفتی بتقمم بسلامت :)' ,message_id=msg.get("message_id"))
							except:
								print("err bye")
					...
					if data["type"]=="RemoveGroupMembers":
						...
						try:
							user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
							bot.sendMessage(target, f"سلام {user} عزیز 😘🌹 \n • به گـروه {name} خوش اومدی", message_id=msg["message_id"])
						except:
